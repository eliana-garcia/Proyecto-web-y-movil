...
const AdminLayout: React.FC<Props> = ({ children }) => {
  ...
  // Usar estado para forzar reactividad en el layout
  ...
  useEffect(() => {
    ...
    setIsAdmin(role === '1');
  }, [location.pathname]); // Re-evaluar al cambiar de ruta

  const visibleMenuItems = menuItems.filter(item => !item.adminOnly || isAdmin);

  const cerrarSesion = () => {
    localStorage.clear();
    // Forzar recarga completa para limpiar estados de Ionic/React en memoria
        ...
        <header className="admin-topbar">
          ...
          <div className="topbar-actions">
            <div className="admin-pill hide-mobile">
              ...
              <span>{userData?.nombre_usuario || userData?.nombre || (isAdmin ? 'Administrador' : 'Funcionario')}</span>
            </div>
