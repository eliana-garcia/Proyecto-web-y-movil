...
/* =========================
   INTEGRACIÃ“N API EXTERNA (EF 5)
   Consumo de Indicadores EconÃ³micos
========================= */

const https = require('https');

app.get("/api/indicadores", verificarToken, (req, res) => {
  ...
  https.get(url, (apiRes) => {
    ...
    apiRes.on('end', () => {
      try {
        ...
        res.status(500).json({ mensaje: "Error al procesar datos de la API externa" });
      }
    });

  }).on("error", (err) => {
    console.error("Error al conectar con mindicador.cl:", err.message);
    res.status(500).json({ mensaje: "Error al conectar con el servicio externo" });
  });
});
